# 📚 Guides d'Installation & Documentation Technique


Bienvenue sur ma **base de connaissances personnelle**.
Ce dépôt a pour but de centraliser, structurer et partager mes procédures d'installation, mes configurations types et mes notes de dépannage pour l'administration système et réseau.

> 💡 *L'objectif : Documenter pour ne jamais oublier et partager pour aider.*

---

## 🛠️ Méthodologie

Chaque guide présent dans ce dépôt suit une structure rigoureuse pour garantir la reproductibilité :
1.  **Prérequis** : Ce qu'il faut avoir avant de commencer.
2.  **Installation** : Les commandes pas à pas.
3.  **Configuration** : Les fichiers à modifier.
4.  **Vérification** : Comment tester que tout fonctionne.

---

## 👤 Auteur

Maintenu par **Paulo Rosa**.

*N'hésitez pas à explorer le code ou à suggérer des améliorations !*
